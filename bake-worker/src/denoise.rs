...
type NewFilter = unsafe extern "C" fn(device: *mut std::ffi::c_void, type_: *const std::ffi::c_char) -> *mut std::ffi::c_void;
...
type SetFilterBool = unsafe extern "C" fn(filter: *mut std::ffi::c_void, name: *const std::ffi::c_char, value: bool);
...
type GetDeviceError =
    unsafe extern "C" fn(device: *mut std::ffi::c_void, out_message: *mut *const std::ffi::c_char) -> i32;
...
impl Oidn {
    ...
    pub fn denoise_gray(&self, values: &mut [f32], width: usize, height: usize) -> Result<(), String> {
        unsafe { self.denoise_gray_raw(values, width, height) }
    }

    unsafe fn denoise_gray_raw(&self, values: &mut [f32], width: usize, height: usize) -> Result<(), String> {
        let lib = &self._lib;
        ...
        let set_filter_bool: Symbol<SetFilterBool> = lib.get(b"oidnSetFilterBool\0").map_err(to_str)?;
        let execute_filter: Symbol<ExecuteFilter> = lib.get(b"oidnExecuteFilter\0").map_err(to_str)?;
        let release_filter: Symbol<ReleaseFilter> = lib.get(b"oidnReleaseFilter\0").map_err(to_str)?;
        let release_device: Symbol<ReleaseDevice> = lib.get(b"oidnReleaseDevice\0").map_err(to_str)?;
        let get_device_error: Symbol<GetDeviceError> = lib.get(b"oidnGetDeviceError\0").map_err(to_str)?;

        let device = new_device(OIDN_DEVICE_CPU);
        if device.is_null() {
            return Err("降噪组件初始化失败（CPU 设备不可用）".into());
        }
        ...
        let error = get_device_error(device, std::ptr::null_mut());
        if error != 0 {
            release_filter(filter);
            release_device(device);
            return Err(format!("AI 降噪执行出错（OIDN 错误码 {error}），请增大 AO 采样数或关闭降噪。"));
        }
