...
- 模型烘焙：从 OBJ/GLB/glTF 模型一键烘焙 AO、厚度、曲率、位置、法线等 Mesh Map，DXR 光线追踪加速 + Intel OIDN AI 降噪，SP 式按材质输出并回写烘焙模型
...
| 文件 | 说明 |
|---|---|
| [AIAS_5.6.2_x64-setup.exe](https://github.com/AvroraCL/AIAS/releases/download/v5.6.2/AIAS_5.6.2_x64-setup.exe) | 中文安装向导，按当前用户安装，无需管理员权限 |

v5.6.2 安全加固：ORT CPU 运行库下载固化官方 SHA256（原先零校验，下载源被劫持可致代码执行）；涂装删除/改名限定在涂装目录内（消除渲染层任意目录删除原语）。
...
├── scripts/model-bake.js          # 模型烘焙工作台（three.js 视图 / 材质列表 / UV 检查）
└── styles/                        # 样式与性能模式样式
src-tauri/src/
...
| 模型烘焙 | 导入 OBJ/GLB 并选择输出 | Mesh Map 按材质输出，烘焙模型可回导 |
