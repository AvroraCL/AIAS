...
AIAS 把 PBR 通道合成与拆分、Mipmap 生成、DDS 批量转换、风格实验室（29 套图片风格化）、AI 抠图与超分、模型烘焙和涂装管理装进一个开箱即用的 Windows 应用，无需命令行，也不依赖 ComfyUI、Python 环境或在线服务；图像编解码与模型推理全部在本机离线完成。

**[下载 Windows 版 v5.7.0](https://github.com/AvroraCL/AIAS/releases/download/v5.7.0/AIAS_5.7.0_x64-setup.exe)** · [English README](README.en.md) · [反馈问题](https://github.com/AvroraCL/AIAS/issues)

![AIAS BLK 生成工作区：使用示例 DDS 展示贴图缩略图与映射规则](docs/aias-blk-workspace.png)

从贴图到可用涂装：**合成 PBR 通道**生成 `_c` / `_n` DDS → **生成 BLK**关联游戏原贴图 → **涂装打包**检查并输出 ZIP → **检查与管理涂装**。需要智能材质时，还可从静态模型烘焙 AO、法线、曲率等 Mesh Maps。上图使用程序生成的示例 DDS，展示实际工作区。
...
- 涂装打包：检查所选 BLK 及其引用的直属目录 DDS/TGA，指出缺失、大小写不符或损坏的文件；仅将这些文件放入单层涂装文件夹并生成可分享的 ZIP。额外文件和投稿相关建议不会阻止普通打包
- Mipmap 生成：把分层图片序列组装为带完整 Mipmap 链的单个 DDS，支持 8K→6K/4K 等游戏友好缩放
- 图片转 DDS：批量转换图片并统一 BCn 压缩格式
- 风格实验室：29 套图片风格化一键出图——图像转 ASCII（字符/方块/波点/线稿）与 25 套其他风格（包括故障艺术、像素排序、坏块流动、信号撕裂、数据腐蚀、CRT 显像管、信号重影、迷彩生成、油画厚涂、半调印刷、素描炭笔、热感假彩、十字绣、双色调、水彩晕染、低多边形、像素画、版画木刻、胶片颗粒、水波纹、玻璃折射、马赛克拼贴、热浪扭曲、蜡笔粉彩、幻彩全息），全部本地实时预览；数值可直接键入、标签拖动微调、双击单参复位、滚轮步进、随机探索，每套附出厂预设并支持自定义预设
...
| 文件 | 说明 |
|---|---|
| [AIAS_5.7.3_x64-setup.exe](https://github.com/AvroraCL/AIAS/releases/download/v5.7.3/AIAS_5.7.3_x64-setup.exe) | Windows 安装包，按当前用户安装无需管理员权限 |
...
AIAS 的贴图管线（PBR 合成 / 拆分、Mipmap、DDS 转换）与涂装管理已稳定可用，适合涂装作者日常使用。AI 抠图已接入 6 个离线模型并持续精修边缘质量——受推理分辨率限制，极细发丝与线稿边界仍有约 2-3 像素的精度上限，导出前建议先在对比视图中确认效果。模型烘焙（AO / 厚度 / 曲率 / 位置等 Mesh Map 一键离线烘焙）已进入可用状态：DXR 光线追踪加速、AI 降噪、SP 式按材质出图、自动 UV 展开带失败回退，详见下文「模型烘焙」。风格实验室已收录 29 套风格并在持续扩充，实时预览与导出在浏览器与桌面端均可使用。

### 模型烘焙

模型烘焙用于从当前静态模型生成 Substance Painter 等智能材质工作流所需的 Mesh Maps，不是高模向低模投射。源模型始终只读，烘焙不会覆盖原始 OBJ、MTL、GLB 或 glTF。

- **导入与 UV**：支持 OBJ、GLB 和 glTF；OBJ 缺少或部分缺少 MTL 时会保留 `usemtl` 材质槽继续导入。默认保留原生 UV，包括负坐标和大于 1 的平铺坐标；烘焙时按重复寻址映射到贴图。仅在 UV 缺失或坐标无效时自动生成工作通道，源文件不变；也可选择全部重算或严格使用源 UV。镜像/分层重叠与零面积退化面不会触发重排。
- **输出贴图**：按材质生成 AO、切线法线、世界空间法线、曲率、位置、厚度、材质 ID 和 UV 检查图。AO 可使用 Intel OIDN 降噪；AO 与厚度可使用 DXR 1.1 光线追踪。输出仅含贴图与清单，供智能材质制作使用，不含模型。
- **原生 UV 复用提示**：结果会统计每个材质在实际贴图分辨率下被多个三角面共用的像素。模型原生 UV 不被更改；如果不同表面共用同一像素，单张 AO、曲率、厚度、位置或世界法线图无法同时表达各面的独立几何信息。有复用的材质会附带黑白 `uv_unique_mask` 蒙版：白色为唯一映射区域，黑色为共用或未覆盖区域。启用相应输出时还会导出 `ao_unique`（共用区域为无遮挡白色）、`curvature_unique`（共用区域为中性曲率且邻域计算只参考同对象、同材质、几何边连通的唯一映射像素；按平铺 UV 跨贴图边缘取邻居）或 `thickness_unique`（共用区域为零厚度）。这些图仅在唯一映射区域保留几何信息；中性区域不能恢复原几何细节。`world_normal_unique` 与 `position_unique` 保留唯一映射区域的原值，在共用纹素及没有可靠留边来源的背景写入中性占位色并将 alpha 设为 0；可靠像素扩展出的留边继承其数值和有效性。制作材质时必须使用 alpha 或 `uv_unique_mask` 排除无效像素。可靠 AO 在降噪前即排除共用区域的冲突值，避免错误阴影扩散到邻近像素；模型着色预览优先使用它。原始贴图仍保留供检查。
...
├── scripts/style-lab*.js          # 风格实验室：29 套风格化效果
...
| 风格实验室 | 导入图片后切换 29 套风格并调参 | 预览实时刷新，数值键入/微调/随机探索生效，导出 PNG |
