...
- **原生 UV 复用提示**：结果会统计每个材质在实际贴图分辨率下被多个三角面共用的像素。模型原生 UV 不被更改；如果不同表面共用同一像素，单张 AO、厚度、位置或世界法线图无法同时表达各面的独立几何信息。烘焙这些几何图时，显著复用的材质会附带黑白 `uv_unique_mask` 蒙版：白色为唯一映射区域，黑色为共用或未覆盖区域，供智能材质限制几何图作用范围。
