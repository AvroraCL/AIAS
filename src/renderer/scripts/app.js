...
const modeMeta = {
  ...
  "stylize-pixelsort": { title: "像素排序", description: "按亮度阈值排序行内像素段，数据融化般的流动条纹" },
  "stylize-datamosh": { title: "坏块流动", description: "模拟视频编码坏块拖影，画面如编码损坏般融化流动" },
  "stylize-tear": { title: "信号撕裂", description: "水平撕裂带错位与通道色散，VHS 跳带的信号失控感" },
  "stylize-decay": { title: "数据腐蚀", description: "低色深量化、随机错扫与块级损坏的数字腐烂风格" },
  "stylize-crt": { title: "CRT 显像管", description: "弧面畸变、荫罩栅格、辉光暗角与滚动亮带的老式显示器质感" },
  "stylize-echo": { title: "信号重影", description: "多份错位半透明残影叠加，天线不良般的迷幻重影" },
  ...
  "stylize-duotone": { title: "双色调", description: "暗部/亮部双色映射与软过渡，海报级双色调输出" },
  "stylize-watercolor": { title: "水彩晕染", description: "fBm 驱动的多遍颜料浸润偏移与纸纹" },
  "stylize-lowpoly": { title: "低多边形", description: "三角网格均值取色与抖动，低多边形海报风" },
  "stylize-pixel": { title: "像素画", description: "降采样 + 色阶量化 + 调色板量化的复古像素画" },
  "stylize-woodcut": { title: "版画木刻", description: "平行线雕版阴影与对比拉伸的木刻版画" },
  "stylize-film": { title: "胶片颗粒", description: "复合噪点、光晕、褪色与暖调的胶片质感" },
  "stylize-ripple": { title: "水波纹", description: "正弦波纹偏移采样的水面折射" },
  "stylize-glass": { title: "玻璃折射", description: "模糊灰度梯度驱动的厚玻璃折射变形" },
  "stylize-mosaic": { title: "马赛克拼贴", description: "瓦片量化、缝隙灌浆与抖动的马赛克拼贴" },
  "stylize-heatwave": { title: "热浪扭曲", description: "正弦列偏移模拟热空气上升的视觉扭曲" },
  "stylize-pastel": { title: "蜡笔粉彩", description: "柔化、蜡笔颗粒、亮部晕开与纸纹的粉彩质感" },
  "stylize-holo": { title: "幻彩全息", description: "亮度驱动的全息幻彩渐变与扫描线" },
...
const modeRegistry = {
  ...
  merge: {
    ...
    blocker() {
      ...
      if (outputBlocker("merge")) return outputBlocker("merge");
      return null;
    },
    drop(paths) {
  ...
  split: {
    ...
    blocker() {
      ...
      if (outputBlocker("split")) return outputBlocker("split");
      return null;
    },
    drop(paths) {
  ...
  mipmap: {
    ...
    blocker() {
      ...
      if (outputBlocker("mipmap")) return outputBlocker("mipmap");
      return null;
    },
    drop(paths) {
  ...
  "image-dds": {
    ...
    blocker() {
      ...
      if (outputBlocker("image-dds")) return outputBlocker("image-dds");
      return null;
    },
    drop(paths) {
  ...
  "stylize-pixelsort": stylizeRegistryEntry(),
  "stylize-datamosh": stylizeRegistryEntry(),
  "stylize-tear": stylizeRegistryEntry(),
  "stylize-decay": stylizeRegistryEntry(),
  "stylize-crt": stylizeRegistryEntry(),
  "stylize-echo": stylizeRegistryEntry(),
  ...
  "stylize-duotone": stylizeRegistryEntry(),
  "stylize-watercolor": stylizeRegistryEntry(),
  "stylize-lowpoly": stylizeRegistryEntry(),
  "stylize-pixel": stylizeRegistryEntry(),
  "stylize-woodcut": stylizeRegistryEntry(),
  "stylize-film": stylizeRegistryEntry(),
  "stylize-ripple": stylizeRegistryEntry(),
  "stylize-glass": stylizeRegistryEntry(),
  "stylize-mosaic": stylizeRegistryEntry(),
  "stylize-heatwave": stylizeRegistryEntry(),
  "stylize-pastel": stylizeRegistryEntry(),
  "stylize-holo": stylizeRegistryEntry(),
  "anime-cutout": {
    ...
    blocker() {
      ...
      if (outputBlocker("anime-cutout")) return outputBlocker("anime-cutout");
...
function superresRegistryEntry(model) {
  return {
    ...
    blocker() {
      ...
      const outputError = outputBlocker(`superres-${model}`);
...
const state = {
  ...
  superresResultEpoch: 0,
  superresActiveIndex: 0,
  superresComparePos: 50,
  skinsCache: [],
  activeMode: "merge",
  activityCount: 0,
  lastOutputPath: "",
...
function getModeOutputPath(mode = state.activeMode) {
  ...
  if (mode === "mipmap") return effectiveOutputDir(stored, $("mipmap-input")?.value, true);
  if (mode === "image-dds") return effectiveOutputDir(stored, state.imageFiles[0]);
  if (mode === "anime-cutout") return effectiveOutputDir(stored, state.animeFiles[0]);
  if (mode.startsWith("superres")) return effectiveOutputDir(stored, state.superresFiles[0]);
  if (mode === "normal-map" || mode === "height-map") return materialMapsUI?.outputPath() || "";
  return stored;
}

function outputBlocker(mode) {
  if (getModeOutputPath(mode)) return null;
  return state.settings?.outputStrategy === "fixed"
    ? "请在设置中选择默认输出目录。"
    : state.settings?.outputStrategy === "input"
      ? "请先选择输入文件或目录。"
      : "请选择输出文件夹。";
}
...
// 下载并发互斥：GPU 运行库/动漫模型/发丝模型/超分模型共用同一把锁，except 排除自身
function downloadBusy(except) {
  return Boolean(
    (except !== "gpu" && state.gpuDownloading) ||
    (except !== "anime" && state.animeDownloading) ||
    (except !== "hair" && state.animeHairDownloading) ||
    (except !== "superres" && state.superresDownloadingId)
  );
}
...
function applyAnimeOutputs(paths, requestKeys = null) {
  ...
  for (const path of paths || []) {
    ...
    const pathStem = animeStem(path).replace(/-\d+$/, ""); // 自动避让同名文件时去掉输出后缀
...
function probeAnimeResult(file) {
  ...
  const dir = getModeOutputPath("anime-cutout");
  ...
  api.filesExist(paths).then((flags) => {
    if (getModeOutputPath("anime-cutout") !== dir || !state.animeFiles.includes(file) || state.animeResults.has(key)) return;
...
// 内联宽高，确保 CSS 铺满生效。抠图与超分共用。
function fitCompareFrame(frameId) {
  const frame = $(frameId);
  if (!frame) return;
...
function fitAnimeCompareFrame() {
  fitCompareFrame("anime-compare-frame");
}
...
// 抠图与超分共用同一套对比滑块交互：拖拽分割线定位、方向键微调
// （Shift 加速、Home/End 到两端），无结果/无本地文件时不响应。
function bindCompareStage(stageId, dividerId, frameId, readPosition, setPosition) {
  const stage = $(stageId);
  ...
  const positionFromEvent = (event) => {
    ...
    if (!rect.width) return readPosition();
  ...
  divider.addEventListener("keydown", (event) => {
    ...
    if (event.key === "ArrowLeft") setPosition(readPosition() - step);
    else if (event.key === "ArrowRight") setPosition(readPosition() + step);
  ...
  window.addEventListener("resize", () => fitCompareFrame(frameId));
}

function bindAnimeGallery() {
  bindCompareStage("anime-compare", "anime-compare-divider", "anime-compare-frame", () => state.animeComparePos, setAnimeComparePosition);
}

function bindSuperresCompare() {
  bindCompareStage("superres-compare", "superres-compare-divider", "superres-compare-frame", () => state.superresComparePos, setSuperresComparePosition);
}
...
function applySuperresOutputs(paths, requestKeys) {
  // 同名输出重跑后内容已更新：换时间戳强制对比图重新加载。
  state.superresResultEpoch = Date.now();
  for (const path of paths || []) {
    // 输出名固定为 {stem}_{倍率}x_{模型id}.png，按后缀还原倍率与模型 id
    const name = basename(path);
    const match = name.match(/^(.*)_([2-4])x_(anime|general)(?:-\d+)?\.png$/i);
    if (!match) continue;
    const stem = match[1];
...
// 批量探测：一次 filesExist 覆盖全部待探测的 (文件, 模型) 组合，避免逐项 IPC
// 与 miss 后每次渲染重探的近似 O(N²) 往返
function probeSuperresBatch(files) {
  const dir = getModeOutputPath(state.activeMode.startsWith("superres") ? state.activeMode : "superres-general");
  if (!dir || !isTauriRuntime) return;
  ...
  for (const file of files || []) {
    for (const modelId of ["anime", "general"]) {
      const key = superresResultKey(file, modelId, scale);
      if (state.superresResults.has(key) || state.superresProbed.has(key)) continue;
      pending.push({ file, modelId, key, candidate: `${dirTrimmed}/${animeStem(file)}_${scale}x_${modelId}.png` });
    }
  }
  ...
  api.filesExist(pending.map((item) => item.candidate)).then((flags) => {
    if ((state.superresPreviewRevision || 0) !== revision || getModeOutputPath(state.activeMode.startsWith("superres") ? state.activeMode : "superres-general") !== dir) {
    ...
    pending.forEach((item, index) => {
      ...
      if (flags[index]) {
        state.superresResults.set(item.key, item.candidate);
        markSuperresResultFound(item.file, item.modelId);
      } else {
        // 结果文件还没出现（任务可能正在运行），允许之后的渲染重新探测。
        state.superresProbed.delete(item.key);
      }
    });
  }).catch(() => {
    if ((state.superresPreviewRevision || 0) !== revision) return;
    for (const item of pending) state.superresProbed.delete(item.key);
  });
}
...
function markSuperresResultFound(file, modelId) {
  const strip = $("superres-thumbs");
  const thumb = strip ? [...strip.children].find((item) => item?.dataset?.path === file) : null;
  const badge = thumb?.querySelector(`.thumb-badge[data-model="${modelId}"]`);
  if (badge) {
  ...
  // 命中当前活动图的对比目标时刷新对比舞台（与动漫画廊同一语义）。
  if (state.superresFiles[state.superresActiveIndex] === file) renderSuperresCompare(file);
}

// 当前应展示的结果：优先当前模式的模型，其次另一模型（两模型常只跑其一）。
function superresPreviewPath(file) {
  const activeModel = state.activeMode === "superres-anime" ? "anime" : "general";
  const other = activeModel === "anime" ? "general" : "anime";
  return state.superresResults.get(superresResultKey(file, activeModel))
    || state.superresResults.get(superresResultKey(file, other))
    || "";
}


function renderSuperresThumbs(files) {
  const strip = $("superres-thumbs");
  if (!strip) return;
  strip.innerHTML = "";
  files.forEach((file, index) => {
    ...
    thumb.type = "button";
    thumb.className = "anime-thumb";
    thumb.classList.toggle("active", index === state.superresActiveIndex);
    thumb.title = file;
    thumb.dataset.path = file;
    ...
    img.alt = basename(file);
    img.draggable = false;
    const resultPath = superresPreviewPath(file);
    ...
    if (fallback) {
      requestGalleryThumbnail(img, thumb, thumbPath, fallback);
    } else {
      thumb.classList.add("placeholder");
    }

    const badges = document.createElement("div");
    badges.className = "superres-badge-group";
    for (const modelId of ["anime", "general"]) {
    ...
    thumb.addEventListener("click", () => {
      state.superresActiveIndex = index;
      renderSuperresGallery();
    });

    thumb.append(img, badges, remove);
    strip.appendChild(thumb);
  });
  refreshIcons(strip);
}

function renderSuperresCompare(file) {
  const stage = $("superres-compare");
  if (!stage || !file) return;
  const result = superresPreviewPath(file);
  const hasResult = Boolean(result);
  const hasLocal = Boolean(superresLocalSrc(file));
  stage.classList.toggle("no-result", !hasResult);
  stage.classList.toggle("no-file", !hasLocal);

  const original = $("superres-compare-original");
  const after = $("superres-compare-after");
  const originalSrc = superresLocalSrc(file);
  const rawResultSrc = superresLocalSrc(result);
  // 输出文件名固定，重跑会覆盖同名文件：换时间戳强制 <img> 重新加载。
  const resultSrc = rawResultSrc && state.superresResultEpoch
    ? `${rawResultSrc}?v=${state.superresResultEpoch}`
    : rawResultSrc;
  if (original) {
    if (originalSrc) original.src = originalSrc;
    else original.removeAttribute("src");
  ...
  if (original) original.onload = () => fitCompareFrame("superres-compare-frame");

  stage.style.setProperty("--compare-pos", hasResult ? `${state.superresComparePos}%` : "100%");
  const hint = $("superres-compare-hint");
  if (hint) hint.hidden = hasResult || !hasLocal;
  const empty = $("superres-compare-empty");
  if (empty) empty.hidden = hasLocal;
  $("superres-compare-divider")?.setAttribute("aria-valuenow", String(Math.round(hasResult ? state.superresComparePos : 100)));
  fitCompareFrame("superres-compare-frame");
}

function setSuperresComparePosition(percent) {
  state.superresComparePos = Math.min(100, Math.max(0, percent));
  $("superres-compare")?.style.setProperty("--compare-pos", `${state.superresComparePos}%`);
  $("superres-compare-divider")?.setAttribute("aria-valuenow", String(Math.round(state.superresComparePos)));
}

function renderSuperresGallery() {
  const files = state.superresFiles;
  if (state.superresActiveIndex >= files.length) state.superresActiveIndex = Math.max(0, files.length - 1);
  const hasFiles = files.length > 0;
  ...
  updateStatus();
  if (!hasFiles) return;
  renderSuperresThumbs(files);
  renderSuperresCompare(files[state.superresActiveIndex]);
  probeSuperresBatch(files);
}
...
async function runSuperres(modelId, button) {
  ...
  const files = [...state.superresFiles];
  const outputPath = getModeOutputPath(`superres-${modelId}`);
  ...
  renderSuperresModelStatus();
  if (result?.outputs?.length && (state.superresPreviewRevision || 0) === revision && getModeOutputPath(`superres-${modelId}`) === outputPath) applySuperresOutputs(result.outputs, requestKeys);
}
...
function renderSkinList(items) {
  ...
  const query = ($("skin-search")?.value || "").trim().toLowerCase();
  const filtered = query ? items.filter((item) => String(item.name || "").toLowerCase().includes(query)) : items;

  if (banner) banner.classList.toggle("hidden", !hasDir);
  if (empty) empty.classList.toggle("hidden", filtered.length > 0);
  if (actionsBar) actionsBar.classList.toggle("hidden", !hasDir);
  if ($("skin-count")) $("skin-count").textContent = query ? `${filtered.length} / ${items.length} 个涂装` : `${items.length} 个涂装`;
  if ($("skin-dir-path")) $("skin-dir-path").textContent = $("skin-path")?.value || "未设置";

  setText("skin-empty-title", hasDir ? (query ? "没有匹配的涂装" : "目录中暂无涂装") : "尚未连接 UserSkins 目录");
  setText("skin-empty-description", hasDir ? (query ? "换个关键词试试" : "导入涂装文件夹后会显示在这里") : "选择目录后即可管理涂装");

  if (!hasDir || !filtered.length) return;
...
async function refreshStoragePanel() {
  if (!isTauriRuntime) {
    ...
    document.querySelectorAll("[data-storage-clean]").forEach(button => { button.disabled = true; });
    return;
  }
  try {
  ...
  for (const entry of entries) {
    ...
    button.addEventListener("click", async () => {
      ...
      try {
        ...
        await Promise.all([refreshAnimeModelStatus(), refreshSuperresModelStatus(), refreshGpuRuntime()]);
...
function applyLanguage(language) {
  localization ??= createLocalization(document.body);
  const resolved = language === "en" ? "en" : "zh-CN";
  localization.setLanguage(resolved);
  document.documentElement.lang = resolved;
  document.querySelectorAll("[data-language]").forEach((button) => {
    button.setAttribute("aria-pressed", String(button.dataset.language === resolved));
  });
...
function effectiveOutputDir(stored, firstInput, inputIsDirectory = false) {
  return resolveOutputDirectory(stored, firstInput, state.settings || {}, inputIsDirectory);
}
...
function applySettingsToForm() {
  ...
  if ($("set-notify-complete")) $("set-notify-complete").checked = settings.notifyOnComplete !== false;
  const map = {
...
function updateStatus() {
  ...
  const outputStrategy = state.settings?.outputStrategy || "ask";
...
function bindTabs() {
  ...
  $("set-output-strategy")?.addEventListener("change", async event => {
    try { state.settings = await api.settings.set({ outputStrategy: event.target.value }); } catch (error) { addActivity("设置保存失败", String(error), "error"); }
    resetAnimeResults(); resetSuperresResults();
    renderAnimeGallery(); renderSuperresGallery();
    materialMapsUI?.refreshOutput(); updateStatus();
  });
...
function bindWorkspaceActions() {
  ...
  $("copy-log")?.addEventListener("click", async () => {
    ...
    if (!text) {
      addActivity("无法复制", "当前模式暂无日志", "idle");
      return;
    }
    try {
...
function bindSettingsActions() {
  ...
  languageButtons.forEach((button) => button.addEventListener("click", async () => {
    ...
    if (next === (state.settings.language === "en" ? "en" : "zh-CN")) return;
    languageButtons.forEach((item) => { item.disabled = true; });
    try {
      state.settings = await api.settings.set({ language: next });
...
function bindRunActions() {
    ...
    await withLog(
      ...
      () =>
        api.texture.mergePbr({
          ...
          outputPath: getModeOutputPath("merge"),
    ...
    await withLog(
      ...
      () =>
        api.texture.splitPbr({
          ...
          outputPath: getModeOutputPath("split"),
    ...
    await withLog(
      ...
      () =>
        api.texture.createMipmap({
          ...
          outputPath: getModeOutputPath("mipmap"),
    ...
    await withLog(
      ...
      () =>
        api.texture.convertImagesToDds({
          ...
          outputPath: getModeOutputPath("image-dds"),
  ...
  bindRunAction("run-anime-cutout", async (button) => {
    ...
    const files = [...state.animeFiles];
    const outputPath = getModeOutputPath("anime-cutout");
    ...
    renderAnimeModelStatus();
    if (result?.outputs?.length && getModeOutputPath("anime-cutout") === outputPath) applyAnimeOutputs(result.outputs, requestKeys);
    refreshGpuRuntime();
  });
...
function bindSkinActions() {
  ...
  $("skin-search")?.addEventListener("input", () => {
    // 搜索同样是纯前端过滤：直接重渲染缓存
    renderSkinList(state.skinsCache);
  });
}
...
async function boot() {
  ...
  materialMapsUI = createMaterialMaps({
    ...
    resolveOutputDir: (stored, input) => effectiveOutputDir(stored, input),
    outputStrategy: () => state.settings?.outputStrategy || "ask",
  ...
  if (state.activeMode === "settings") refreshStoragePanel();
...
if (import.meta.hot) import.meta.hot.dispose(() => asciiUI?.dispose());
