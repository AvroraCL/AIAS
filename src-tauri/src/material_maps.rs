...
fn coord(i: i64, len: u32, wrap: bool) -> u32 {
    if wrap {
        i.rem_euclid(i64::from(len)) as u32
    } else {
...
fn height_field(source: &DynamicImage, p: &Parameters) -> HeightField {
    ...
    for pixel in rgba.pixels() {
        let value = match p.channel.as_str() {
            ...
            _ => {
                0.2126 * srgb_linear(pixel[0])
                    + 0.7152 * srgb_linear(pixel[1])
                    + 0.0722 * srgb_linear(pixel[2])
            }
        };
    ...
    for (value, a) in values.iter_mut().zip(&alpha) {
        *value = 0.5 + (*value - 0.5) * a;
    }
    HeightField {
        width: w,
        height: h,
...
fn smooth_height(
    ...
    for vertical in [false, true] {
        ...
        for line in 0..lines {
            ...
            for offset in -r..=r {
                sum_v += weighted[index(offset)] as f64;
                sum_a += weights[index(offset)] as f64;
            }
            ...
            for pos in 0..length {
                ...
                sum_v += weighted[index(pos as i64 + r + 1)] as f64
                    - weighted[index(pos as i64 - r)] as f64;
                sum_a += weights[index(pos as i64 + r + 1)] as f64
                    - weights[index(pos as i64 - r)] as f64;
            }
        }
        weighted = out_v;
        weights = out_a;
...
fn normals(field: &HeightField, p: &Parameters) -> RgbImage {
    ...
    RgbImage::from_fn(w, h, |x, y| {
        ...
        let sample = |sx: i64, sy: i64| {
            ...
            let center = if center_a > 1e-6 {
                (field.values[i] - 0.5) / center_a + 0.5
            } else {
                0.5
            };
            let height = if a > 1e-6 {
                (field.values[j] - 0.5) / a + 0.5
            } else {
                center
            };
...
fn height_image(field: &HeightField, bits: u8) -> DynamicImage {
    if bits == 8 {
        DynamicImage::ImageLuma8(ImageBuffer::from_fn(field.width, field.height, |x, y| {
            Luma([(field.values[(y * field.width + x) as usize] * 255.0).round() as u8])
        }))
    } else {
        DynamicImage::ImageLuma16(ImageBuffer::from_fn(field.width, field.height, |x, y| {
            Luma([(field.values[(y * field.width + x) as usize] * 65535.0).round() as u16])
        }))
    }
}
...
fn decode(path: &Path) -> Result<DynamicImage, String> {
    let extension = path
        ...
        .unwrap_or("")
        .to_lowercase();
    if !["png", "jpg", "jpeg", "webp", "tga"].contains(&extension.as_str()) {
        return Err("支持 PNG、JPEG、WebP、TGA 素材。".into());
    }
    ...
    if let Some(orientation) = crate::anime::exif_orientation(path)? {
...
#[tauri::command]
pub(crate) async fn material_maps_preview(
    input: String,
...
fn preview(
    ...
    let source = if image.width().max(image.height()) > 1024 {
        image.resize(1024, 1024, image::imageops::FilterType::Triangle)
    } else {
        image
    };
    let field = height_field(&source, parameters);
    Ok(Preview {
        source: if wanted("source") {
            png_data(&source)?
        } else {
            String::new()
        },
        normal: if wanted("normal") {
            png_data(&DynamicImage::ImageRgb8(normals(&field, parameters)))?
        } else {
            String::new()
        },
        height: if wanted("height") {
            png_data(&height_image(&field, 8))?
        } else {
            String::new()
        },
        width: source.width(),
        height_pixels: source.height(),
    })
}
...
fn stem(input: &Path) -> Result<String, String> {
    let name = input
        ...
        .and_then(|v| v.to_str())
        .ok_or("素材文件名无效")?;
    let prefix = if name.to_lowercase().ends_with("_basecolor") {
        &name[..name.len() - "_basecolor".len()]
    } else {
        name
    };
    if prefix.is_empty() {
        return Err("素材名称不能只有 _basecolor。".into());
    }
    Ok(prefix.into())
}
...
fn output_plan(options: &RunOptions, policy: &str) -> Result<Vec<Vec<PathBuf>>, String> {
    ...
    let inputs: HashSet<String> = options
        .files
        .iter()
        .map(|f| {
    ...
    options
        ...
        .map(|input| {
            ...
            let kinds = if options.kind == "normal" && options.parameters.also_height {
                vec!["normal", "height"]
            } else {
                vec![options.kind.as_str()]
            };
            kinds
                .into_iter()
                .map(|kind| {
                    ...
                    if !seen.insert(key.clone()) {
                        return Err(format!(
                            "输出文件重名：{name}_{kind}.png，请重命名或分批导出。"
                        ));
                    }
                    ...
                    if inputs.contains(&resolved) {
                        return Err(format!("输出会覆盖输入素材：{}", path.display()));
                    }
                    Ok(path)
                })
                .collect()
...
fn generate(app: Option<&AppHandle>, options: RunOptions) -> Result<crate::TaskResult, String> {
    ...
    for (index, (input, targets)) in options.files.iter().zip(plan).enumerate() {
        ...
        let progress = |fraction: f64, phase: &str| {
            if let Some(app) = app {
                crate::emit_task_progress_percent(
                    app,
                    index,
                    options.files.len(),
                    format!(
        ...
        let result = (|| {
            ...
            progress(0.6, "生成贴图");
            let normal = if options.kind == "normal" {
                Some(DynamicImage::ImageRgb8(normals(
                    &field,
                    &options.parameters,
                )))
            } else {
            ...
            let height = if options.kind == "height" || options.parameters.also_height {
                Some(height_image(&field, options.parameters.bits))
            } else {
                None
            };
        ...
        match result {
            Ok(()) => {
                completed += 1;
                crate::push_log(app, &mut logs, "success", format!("完成 {input}"));
            }
            Err(error) => {
                crate::push_log(app, &mut logs, "error", format!("失败 {input}：{error}"))
            }
        }
...
#[tauri::command]
pub(crate) async fn material_maps_generate(
    app: AppHandle,
    options: RunOptions,
...
mod tests {
    ...
    fn area() -> tempfile::TempDir {
        let root = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .unwrap()
            .join("测试区/临时输出/material-map-tests");
    ...
    fn flat_single_pixel_and_zero_strength_are_safe() {
        ...
        assert!(normals(
            &height_field(&gray(9, 9, |x, y| ((x + y) * 3000) as u16), &p),
            &p
        )
        .pixels()
        .all(|p| p.0 == [128, 128, 255]));
    }
    #[test]
    fn ramps_conventions_and_inversion_have_correct_directions() {
        ...
        assert!(gl.get_pixel(4, 4)[0] < 128);
        assert!(gl.get_pixel(4, 4)[1] > 128);
        p.convention = "directx".into();
        let dx = normals(&height_field(&source, &p), &p);
        for (a, b) in gl.pixels().zip(dx.pixels()) {
            assert_eq!(a[0], b[0]);
            assert_eq!(a[2], b[2]);
            assert_eq!(a[1], 255 - b[1]);
        }
        p.convention = "opengl".into();
        p.invert = true;
        let reversed = normals(&height_field(&source, &p), &p);
        assert!(reversed.get_pixel(4, 4)[0] > 128);
        assert!(reversed.get_pixel(4, 4)[1] < 128);
    }
    #[test]
    fn sixteen_bit_grayscale_roundtrips_without_gamma_or_normalization() {
        ...
        let rgb =
            DynamicImage::ImageRgb16(ImageBuffer::from_pixel(1, 1, Rgb([12345u16, 20000, 40000])));
        assert_eq!(
            height_image(&height_field(&rgb, &p), 16)
                .as_luma16()
                .unwrap()
                .get_pixel(0, 0)[0],
            12345
    ...
    fn transparent_color_does_not_create_rims() {
        let source = DynamicImage::ImageRgba8(image::RgbaImage::from_fn(12, 8, |x, _| {
            if x < 6 {
        ...
        for smoothing in [0, 3, 20] {
            ...
            let field = height_field(&source, &p);
        ...
        assert!(
            (height_field(&source, &Parameters::default()).values[0] - (0.5 + 0.5 * 128.0 / 255.0))
                .abs()
                < 1e-6
        );
    }
    ...
    fn wrapping_samples_opposite_edge() {
        assert_eq!(coord(-1, 5, true), 4);
        assert_eq!(coord(5, 5, true), 0);
        let source = gray(5, 1, |x, _| (x * 12000) as u16);
        let p = Parameters::default();
        let clamped = normals(&height_field(&source, &p), &p);
        let p = Parameters {
            boundary: "wrap".into(),
            ..p
        };
        let wrapped = normals(&height_field(&source, &p), &p);
        assert!(clamped.get_pixel(0, 0)[0] < 128);
        assert!(wrapped.get_pixel(0, 0)[0] > 128);
    }
    #[test]
    fn invalid_parameters_are_rejected() {
        for value in [f32::NAN, f32::INFINITY, -1.0, 11.0] {
            assert!(Parameters {
                strength: value,
                ..Default::default()
            }
    ...
    fn preflight_handles_final_names_and_input_conflicts() {
        let temp = area();
        let root = temp.path();
        for name in ["hero.png", "hero_basecolor.png", "hero_normal.png"] {
            gray(2, 2, |_, _| 32768).save(root.join(name)).unwrap();
        }
        let mut options = RunOptions {
            files: vec![
                root.join("hero.png").display().to_string(),
                root.join("hero_basecolor.png").display().to_string(),
            ],
            output_path: root.display().to_string(),
            kind: "normal".into(),
    ...
    fn generation_writes_sixteen_bit_height_and_pbr_named_normal() {
        ...
        let temp = area();
        let input = temp.path().join("tile_basecolor.png");
        gray(7, 5, |x, y| (10000 + x * 700 + y * 300) as u16)
            .save(&input)
            .unwrap();
        let result = generate(
            None,
        ...
        assert!(result
            .outputs
            .iter()
            .any(|p| p.ends_with("tile_normal.png")));
        assert_eq!(
            image::open(temp.path().join("tile_height.png"))
                .unwrap()
                .color(),
            image::ColorType::L16
        );
        assert_eq!(
            image::open(temp.path().join("tile_normal.png"))
                .unwrap()
                .color(),
    ...
    fn cancel_during_batch_stops_at_file_boundary_and_keeps_results() {
        ...
        for index in 0..60 {
            ...
            gray(128, 128, |x, y| (x * 300 + y * 200) as u16)
                .save(&path)
                .unwrap();
        ...
        assert!(
            result.logs.iter().any(|line| line.contains("任务已停止")),
            "停止提示应进入日志"
        );
    }
    ...
    fn real_material_export() {
        ...
        let field = height_field(&source, &params);
        source.save(out.join("preview-source.png")).unwrap();
        normals(&field, &params)
            .save(out.join("preview-normal.png"))
            .unwrap();
        height_image(&field, 8)
            .save(out.join("preview-height.png"))
            .unwrap();
        ...
        let group = out.join("pbr-integration");
        ...
        for name in [
            ...
            RgbImage::from_pixel(4, 4, Rgb([128, 128, 255]))
                .save(group.join(name))
                .unwrap();
        }
        std::fs::copy(
            out.join("142101839_p0_normal.png"),
            group.join("tile_normal.png"),
        )
        .unwrap();
        ...
        assert_eq!(
            image::open(out.join("142101839_p0_height.png"))
                .unwrap()
                .color(),
            image::ColorType::L16
        );
        assert_eq!(
            image::open(out.join("142101839_p0_normal.png"))
                .unwrap()
                .dimensions(),
            (1676, 2000)
        );
