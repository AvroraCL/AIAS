...
3. **Package the skin:** check the chosen BLK and its directly referenced DDS/TGA files, then create a shareable ZIP containing only those files in one top-level folder. Missing, mis-cased, unsafe or damaged references block packaging; extra files and submission advice do not.
4. **Check and manage the skin:** use the built-in skin manager to find, import and enable War Thunder UserSkins.

For smart-material workflows, the model baker generates AO, normal, curvature, position, thickness, ID and UV Mesh Maps from the **same static model**. It is not a high-poly-to-low-poly projection baker. OBJ, GLB and glTF import are supported; the source model is not overwritten. Native tiled UVs are preserved. Results report texture pixels shared by multiple surfaces. Materials with UV reuse receive an extra `uv_unique_mask` PNG (white = uniquely mapped, black = shared or uncovered). When enabled, AO, curvature and thickness also receive `ao_unique` (unoccluded white in shared regions), `curvature_unique` (neutral curvature in shared regions; neighbors must be uniquely mapped and connected by mesh edges within the same object and material, including across periodic texture edges), and `thickness_unique` (zero in shared regions). These maps retain geometry information only in uniquely mapped regions; neutral areas cannot recover the original surface detail. `world_normal_unique` and `position_unique` preserve original values on uniquely mapped pixels; shared pixels and background without a reliable margin source use neutral placeholder RGB with zero alpha, while margin inherited from a unique pixel remains valid. Material authoring must honor that alpha or `uv_unique_mask` to exclude invalid data. Reliable AO excludes conflicting pixels before denoising so false shadows cannot spread from them to nearby pixels. The shaded preview prefers reliable AO; raw maps remain available for inspection.
